...
class PostgresPreparedStatement implements CapPreparedStatement {
  ...
  async run(...params: SqlValue[]) {
    ...
    const rawInsertId = insertIdTable && rows[0] ? rows[0]?.id : undefined;
    const parsedInsertId =
      rawInsertId === undefined || rawInsertId === null ? undefined : Number(rawInsertId);
    const lastInsertRowid = Number.isFinite(parsedInsertId) ? parsedInsertId : undefined;
...
function migrateLegacySqliteSchema(database: Database.Database) {
  ...
  const inviteColumns = new Set(
    (database.prepare("PRAGMA table_info('department_invites')").all() as Array<{ name: string }>).map(
      (column) => column.name
    )
  );

  if (!inviteColumns.has('token')) {
    database.exec('ALTER TABLE department_invites ADD COLUMN token TEXT');
  }
...
async function ensureDatabase(database: CapDatabase) {
  ...
  try {
    if (database.driver === 'postgres') {
      await database.exec('ALTER TABLE department_invites ADD COLUMN IF NOT EXISTS token TEXT');
    } else if (database.driver === 'd1-remote') {
      try {
        await database.exec('ALTER TABLE department_invites ADD COLUMN token TEXT');
      } catch {
        // Column already exists.
      }
    }
  } catch {
    // Keep boot resilient across backends.
  }
