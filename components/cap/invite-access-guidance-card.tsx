  ...
  return (
    <article className="space-y-4 rounded-[28px] border border-[#ddd3f0] bg-white p-6 shadow-sm">
      <div>
        ...
        <h3 className="mt-2 text-2xl font-semibold text-[#241c33]">Use a department link to unlock CAP</h3>
        <p className="mt-2 text-sm text-[#5f5673]">
          CAP is opened by department invite link. A chief admin, main admin, or department admin creates the link,
          shares it with the exact ministry member, and the claim flow grants department access automatically.
        </p>
      </div>

      <div className="space-y-3 rounded-2xl border border-[#e6def4] bg-[#fbf9fe] p-4">
        {[
          'Ask the head of department or CAP admin to generate a one-time invite link from the Admin page.',
          'Open that link and sign in with Google or your email/password.',
          'CAP will place you directly into the correct department workflow instead of keeping you in a waiting queue.',
        ].map((item) => (
