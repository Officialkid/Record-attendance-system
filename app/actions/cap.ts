...
async function getEventIdForContributionLedger(ledgerId: number) {
  ...
  const row = (await db
    .prepare('SELECT event_id FROM contribution_ledgers WHERE id = ?')
    .get(ledgerId)) as Record<string, unknown> | undefined;
...
async function getEventIdForContributionParticipant(participantId: number) {
  ...
  const row = (await db
    .prepare(
      `SELECT contribution_ledgers.event_id
...
async function getEventIdForExpenseLedger(ledgerId: number) {
  ...
  const db = await getDb();
  const row = (await db
    .prepare('SELECT event_id FROM expense_ledgers WHERE id = ?')
    .get(ledgerId)) as Record<string, unknown> | undefined;
  return getNumericRowValue(row, 'event_id');
}
...
export async function importProtocolRecordsAction(formData: FormData) {
  try {
    const user = await requireSessionUser();
    const { extractTextFromDocumentFile, parseProtocolAccountsText } = await import('@/lib/cap/document-parser');
    const departmentId = parsePositiveNumber(formData.get('departmentId'), 1);
    const handledByUserId = parsePositiveNumber(formData.get('handledByUserId'), Number(user.id));
    const pastedText = String(formData.get('pastedText') || '').trim();
    const uploadedFile = formData.get('document');

    let sourceText = pastedText;
    if (!sourceText && uploadedFile instanceof File && uploadedFile.size > 0) {
      sourceText = await extractTextFromDocumentFile(uploadedFile);
    }

    if (!sourceText) {
      throw new Error('Paste the extracted table or upload the source document before importing.');
    }

    const rows = parseProtocolAccountsText(sourceText);
    const result = await importDepartmentRecords(user, {
      departmentId,
      handledByUserId,
      rows,
    });
    ...
    return {
      success: true,
      message: `Imported ${result.importedCount} protocol record(s). ${result.createdCount} created, ${result.updatedCount} updated.`,
      ...result,
    };
  } catch (error) {
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Failed to import protocol records.',
    };
  }
}
...
export async function createEventAction(input: Parameters<typeof createEvent>[1]) {
  try {
    ...
    await revalidateProgramsEventPaths(eventId);
    revalidatePath('/dashboard');
...
export async function addContributionParticipantAction(
  ...
  try {
    ...
    const eventId = await getEventIdForContributionLedger(input.ledgerId);
    if (eventId) {
      await revalidateProgramsEventPaths(eventId);
    } else {
      revalidatePath('/programs');
...
export async function recordContributionPaymentAction(
  ...
  try {
    ...
    const eventId = await getEventIdForContributionParticipant(input.participantId);
    if (eventId) {
      await revalidateProgramsEventPaths(eventId);
    } else {
      revalidatePath('/programs');
...
export async function addExpenseCategoryAction(input: Parameters<typeof addExpenseCategory>[1]) {
  try {
    ...
    const eventId = await getEventIdForExpenseLedger(input.ledgerId);
    if (eventId) {
      await revalidateProgramsEventPaths(eventId);
    } else {
      revalidatePath('/programs');
...
export async function addExpenseItemAction(input: Parameters<typeof addExpenseItem>[1]) {
  try {
    ...
    const eventId = await getEventIdForExpenseCategory(input.categoryId);
    if (eventId) {
      await revalidateProgramsEventPaths(eventId);
    } else {
      revalidatePath('/programs');
...
export async function deleteEventAction(eventId: number) {
  try {
    const user = await requireSessionUser();
    await deleteEvent(user, eventId);
    await revalidateProgramsEventPaths(eventId);
    revalidatePath('/dashboard');
